...
Confluence.Templates.Sidebar.renderLinks = function(opt_data, opt_sb) {
  ...
  if (opt_data.quickLinks.length) {
    output.append('<h5>', soy.$$escapeHtml("Bereichsverkn\xfcpfungen"), '</h5>');
    ...
    output.append('<h5>', soy.$$escapeHtml("Bereichsverkn\xfcpfungen"), '</h5><p class="tip">', AJS.format("\x3ca href\x3d\x22{0}\x22 class\x3d\x22{1}\x22\x3eHier\x3c/a\x3e k\xf6nnen Sie Verkn\xfcpfungen zu Ihren wichtigsten Inhalten vornehmen.",'','configure-sidebar'), '</p>');
  }
...
Confluence.Templates.Sidebar.renderLinksSection = function(opt_data, opt_sb) {
  ...
  if (opt_data.links.length) {
    ...
    output.append((opt_data.includeConfigureLink) ? '<li class="acs-nav-item configure"><a id="acs-configure-link" class="acs-nav-item-link" href="" data-collapsed-tooltip="' + soy.$$escapeHtml("Randleiste konfigurieren") + '"><span class="icon"></span><span class="acs-nav-item-label">' + soy.$$escapeHtml("Randleiste konfigurieren") + '</span></a></li>' : '', '</ul></div>');
  }
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.Sidebar.contextualNav = function(opt_data, opt_sb) {
  ...
  if (opt_data.forBlogs) {
    ...
    output.append('<div class="ia-secondary-header"><h5 class="ia-secondary-header-title settings"><span class="label">', soy.$$escapeHtml("Erweitert"), '</span></h5></div><div class="ia-secondary-content">');
    Confluence.Templates.Sidebar.pagetreeList({pagetree: opt_data.contextualNav}, output);
    output.append('</div>');
  } else {
...
Confluence.Templates.Sidebar.throbber = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div class="content-container"><div class="throbber-container"><div class="throbber"><div class="spinner"></div><span>', soy.$$escapeHtml("Wird geladen..."), '</span></div></div></div>');
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.Sidebar.treeThrobber = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('\t<ul class="ia-subpagetree"><li class="acs-tree-item leaf"><span class="node-title">', soy.$$escapeHtml("Wird geladen..."), '</span></li></ul>');
  return opt_sb ? '' : output.toString();
};
...
Confluence.Templates.Sidebar.Pages.render = function(opt_data, opt_sb) {
  ...
  if (opt_data.children.initialPages.length) {
    ...
    if (opt_data.children.remainingPages.length) {
      ...
      output.append('</ul><a class="more-children-link" href=""><span class="icon"></span><span class="label">', soy.$$escapeHtml(AJS.format("{0} weitere untergeordnete",opt_data.children.remainingPages.length)), '</span></a>');
    }
  }
  output.append((opt_data.createPermission && opt_data.children.createLink) ? '<a class="create-child-page-link" href="' + soy.$$escapeHtml(opt_data.children.createLink) + '"><span class="icon"></span><span class="label">' + soy.$$escapeHtml("Untergeordnete Seite erstellen") + '</span></a>' : '');
  return opt_sb ? '' : output.toString();
};
